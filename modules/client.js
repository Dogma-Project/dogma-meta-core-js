...
const client = module.exports = {
	...
	connect: (peer) => {
		try {
			...
			const socket = tls.connect(peer.port, peer.host, getOptions(), () => {
				if (client.permitUnauthorized() || socket.authorized) {
					if (socket.authorized) {
						logger.log("client", "{connected}", "Connection authorized by a CA.");
					} else {
						logger.log("client", "{connected}", "Connection didn't authorized by a CA.");
					}
					connection.onConnect(socket, peer);
				} else {
					return logger.log("client", "Connection not authorized: " + socket.authorizationError);
				}
			});
			socket.on('close', () => {
			...
			socket.on("error", error => {
				logger.log("client", "socket error", error.errno, error.address + ":" + error.port);
			});
		} catch (e) {
	 ...
	 */
	tryPeer(peer, from) {  // check non-listed peers

		if (!peer || !peer.host || !peer.port) return logger.warn("nodes", "unknown peer", peer);
		if (!from || !from.user_id || !from.node_id) return logger.warn("nodes", "unknown from", from);
		...
		if (!client.permitUnauthorized()) {
			...
			if (!inFriends) return logger.log("nodes", "tryPeer", user_id, "not in the friends list");
		}

		const address = host + ":" + port;
		Connection.getConnectionsCount(address, node_id).then((count) => {
...
LocalDiscovery.on("message", (data) => { // validate
...
subscribe(["nodes", "users", "nodes", "node-key"], () => { // edit 
...
subscribe(["config-dhtLookup", "users", "node-key"], () => { // edit
	if (args.discovery) return; // don't lookup in discovery mode
