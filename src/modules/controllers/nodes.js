 ...
 * @param {Array} nodes 
 */
const validateAndFilterNodes = (nodes) => { // edit
...
const nodes = module.exports = {
	...
	*/
	async handleRequest({ node_id, request }) {
		...
		switch (request.action) {
			case "get": // store.nodes
				...
				break;
			case "update":
				if (!request.data || !request.data.nodes) return logger.warn("NODES", "there's no nodes to persist");
