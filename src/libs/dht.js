...
class DHT extends EventEmitter {
	 ...
	 */
	async handleRequest(params, socket) { // add validation 
		try {
			...
			switch (type) {
				case "announce":
					await this._handleAnnounce(params);
					break;
				case "revoke":
					await this._handleRevoke(params);
					break;
				case "lookup":
					if (action === "get") {
						const peers = await this._handleLookup(params);
	...
	_handleAnnounce({ from, request }) {
		...
		return new Promise((resolve, reject) => {
			...
			dht.find(full, (err, result) => { // reject if already present
				if (err) reject({
					error: "find-announce",
					data: err
				});
				...
				dht.update(conditions, full, { upsert: true }, (err, result) => { // update or insert new value
					if (err) reject({
						error: "find-announce",
						data: err
					});
	 ...
	 */
	_dhtMulticast(type, data) { // add validation
		try {
