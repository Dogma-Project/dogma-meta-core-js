...
stateManager.subscribe(
  [C_Event.Type.masterKey, C_Event.Type.prefix],
  ([payload, prefix]) => {
    ...
    if (payload === C_System.States.empty) {
      ...
      if (getArg(C_System.Args.auto)) {
        createKeyPair(C_Keys.Type.masterKey, prefix, 4096)
          .then(() => {
...
stateManager.subscribe(
  [C_Event.Type.nodeKey, C_Event.Type.prefix],
  ([payload, prefix]) => {
    const dir = getDatadir(prefix);
    if (payload === C_System.States.empty) {
      logger.info("KEYS", "Node key not found");
      if (getArg(C_System.Args.auto)) {
        createKeyPair(C_Keys.Type.nodeKey, prefix, 2048)
          .then(() => {
